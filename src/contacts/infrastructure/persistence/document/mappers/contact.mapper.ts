...
export class ContactMapper {
    static toDomain(raw: ContactSchemaClass): Contact {
        ...
        if (raw.createdBy) {
            domainEntity.createdBy = typeof raw.createdBy === 'string' ? raw.createdBy : UserMapper.toDomain(raw.createdBy as any);
        }
        if (raw.updatedBy) {
            domainEntity.updatedBy = typeof raw.updatedBy === 'string' ? raw.updatedBy : UserMapper.toDomain(raw.updatedBy as any);
        }
    ...
    static toPersistence(domainEntity: Contact): ContactSchemaClass {
        ...
        persistenceEntity.owner = (typeof domainEntity.owner === 'string' ? domainEntity.owner : domainEntity.owner?.id) as string | undefined;
        persistenceEntity.createdBy = (typeof domainEntity.createdBy === 'string' ? domainEntity.createdBy : domainEntity.createdBy?.id) as string;
        persistenceEntity.updatedBy = (typeof domainEntity.updatedBy === 'string' ? domainEntity.updatedBy : domainEntity.updatedBy?.id) as string;
