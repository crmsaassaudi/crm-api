...
import { ContactSchemaClass, ContactSchemaDocument } from '../entities/contact.schema';
...
@Injectable()
export class ContactRepository extends BaseDocumentRepository<ContactSchemaDocument, Contact> {
    constructor(
    ...
    async findManyWithPagination({
        ...
        return pagination(docs.map(doc => this.mapToDomain(doc)), totalItems, paginationOptions);
    }

    async findOne(filter: FilterQuery<ContactSchemaClass>): Promise<Contact | null> {
    ...
    async checkDuplicate(params: { email?: string; phone?: string; excludeId?: string }): Promise<Contact[]> {
        const { email, phone, excludeId } = params;
        const where: FilterQuery<ContactSchemaClass> = {};

        const conditions: FilterQuery<ContactSchemaClass>[] = [];
        if (email) conditions.push({ email: { $in: [email] } });
        if (phone) conditions.push({ phone: { $in: [phone] } });
