...
describe('CsvImportParser', () => {
  ...
  it('should reads header row', async () => {
  ...
  it('should strips a UTF-8 BOM from the first header', async () => {
    const headers = await parser.readHeaders(
  ...
  it('should returns [] for an empty file', async () => {
    expect(await parser.readHeaders(streamOf(''))).toEqual([]);
  });

  it('should streams data rows keyed by header', async () => {
  ...
  it('should trims surrounding whitespace from values', async () => {
    const rows: Record<string, string>[] = [];
