...
async function simulate() {
  ...
  const conversation = conversations.find(
    (c: any) => c.customer.externalId === EXTERNAL_ID,
  );

  if (!conversation) {
    console.error(
      '❌ Error: Conversation not found in DB. Did the processor fail?',
    );
    process.exit(1);
  }
  console.log(
    `✅ Success: Found conversation ${conversation._id} (Pillar 5 status: persisted)`,
  );

  // 4. Query Messages
  console.log('💬 Step 3: Verifying message history persistence...');
  const msgRes = await axios.get(
    `${API_URL}/omni/conversations/${conversation._id}/messages`,
  );
  ...
  await axios.patch(
    `${API_URL}/omni/conversations/${conversation._id}/status`,
    {
      status: 'resolved',
    },
  );
  ...
  const updatedConv = await axios.get(
    `${API_URL}/omni/conversations/${conversation._id}`,
  );
  if (updatedConv.data.status === 'resolved') {
    console.log(
      '✅ Success: Conversation status updated via REST API (Pillar 6 status: operational)',
    );
  }
