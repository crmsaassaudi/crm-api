...
async function run() {
  try {
    ...
    const p1 = axios.patch(
      `${BASE_URL}/users/${userId}`,
      { firstName: 'Idempotency1' },
      { headers: headersWithKey },
    );
    const p2 = axios.patch(
      `${BASE_URL}/users/${userId}`,
      { firstName: 'Idempotency2' },
      { headers: headersWithKey },
    );
    ...
    results.forEach((res, i) => {
      if (res.status === 'fulfilled') {
        ...
        console.log(
          `Request ${i + 1}: Failed (Status: ${res.reason.response?.status} - ${res.reason.response?.data?.message})`,
        );
      }
    });
    ...
    console.log(
      'Skipping Optimistic Locking API test as DTO might not contain "version" field. Verification done via Code Review.',
    );
  } catch (error) {
    console.error(
      'Error:',
