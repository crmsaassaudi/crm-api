...
export class TikTokPostPublisher extends BasePublisher {
  ...
  validateContentLimits({ content, mediaUrls, mediaType }: PublishContext['post']): void {
    ...
    if (content.length > 2200) {
      throw new BadRequestException('TikTok captions support up to 2,200 characters.');
    }
    if (mediaType !== 'video' || mediaUrls.length !== 1) {
      throw new BadRequestException('TikTok publishing requires exactly one video.');
    }
  }
