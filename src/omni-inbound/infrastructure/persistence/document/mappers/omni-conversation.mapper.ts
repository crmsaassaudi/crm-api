...
export class OmniConversationMapper {
  static toDomain(raw: OmniConversationSchemaClass): OmniConversation {
    ...
    return {
      ...
      firstRespondedAt: (raw as any).firstRespondedAt ?? null,
      firstResponderId: this.normalizeId((raw as any).firstResponderId),
      queuedAt: (raw as any).queuedAt ?? null,
      assignedAt: (raw as any).assignedAt ?? null,
      totalQueuedMs: (raw as any).totalQueuedMs ?? 0,
      slaDueAt: (raw as any).slaDueAt ?? null,
      slaDueMetric: (raw as any).slaDueMetric ?? null,
      slaBreached: (raw as any).slaBreached ?? false,
      slaBreachedAt: (raw as any).slaBreachedAt ?? null,
