...
import {
  RESILIENCE_POLICIES,
  ResilienceOptions,
  ResilienceServiceType,
} from './resilience.definitions';
...
export class ResilienceService {
  ...
  public getPolicy(
    ...
    const registryDefaults =
      RESILIENCE_POLICIES[serviceName as ResilienceServiceType] ||
      RESILIENCE_POLICIES.generic;
    ...
    retryPolicy.onRetry((reason) => {
      ...
      this.logger.warn(
        `${getContext()} [Retry] Attempt ${reason.attempt}. Error: ${error?.message}`,
      );
    ...
    circuitBreakerPolicy.onBreak((reason) => {
      const error = 'error' in reason ? reason.error : undefined;
      this.logger.error(
        `${getContext()} [CircuitBreaker] OPEN. Error: ${error?.message}`,
      );
    });
    ...
    timeoutPolicy.onTimeout(() => {
      this.logger.error(
        `${getContext()} [Timeout] Request exceeded ${timeoutMs}ms`,
      );
    });
