...
import { AuthGuard, KeycloakConnectConfig, KeycloakMultiTenantService, KEYCLOAK_INSTANCE, KEYCLOAK_CONNECT_OPTIONS, KEYCLOAK_LOGGER } from 'nest-keycloak-connect';
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (sid) {
            try {
                ...
                const decodedToken = this.decodeJwt(session.idToken || session.accessToken);
                if (!decodedToken) {
